...
export default class Blog extends React.Component {
	constructor(props) {
		super(props);
		this.state = { currentSocial: "twitter", blogIndex: 0, twitterLoaded: true, facebookLoaded: false, youtubeLoaded: false, linkedinLoaded: false };
	...
	arrowUpClickHandler() {
		if (this.state.blogIndex > 0) this.setState((prevState) => this.setState({ blogIndex: prevState.blogIndex - 1 }));
	}

	arrowDownClickHandler() {
		if (this.state.blogIndex < this.props.blogs.length - 1) this.setState((prevState) => this.setState({ blogIndex: prevState.blogIndex + 1 }));
	}
			...
			<div className={style["blog"]}>
				...
				<div className={style["blog-content"]}>
					...
					<div className={style["blog-view"]}>
						{this.props.blogs == null && <Glimmer />}
						{this.props.blogs != null && this.props.blogs.length === 0 && <p className={style["blog-noitems"]}>gli articoli arriveranno presto</p>}
						{this.props.blogs != null && this.props.blogs.map((blog, index) => (
							(index >= this.state.blogIndex && index < this.state.blogIndex + VISIBLE_BLOGS_COUNT) && <div key={blog.attributes.titolo} className={style["blog-item"]}>
								<a className={style["blog-item__link"]} href={blog.attributes.collegamentoweb ? blog.attributes.collegamentoweb : "#!"} target="_blank" rel="noreferrer">
									<img src={siteUrls.siteUrl + blog.attributes.immagine?.data?.attributes?.url} className={style["blog-item__image"]} alt={blog.attributes.titolo} />
									<div className={style["blog-info"]}>
										<span className={style["blog-info__category"]}>{blog.attributes.tipa}</span>
										<h3 className={style["blog-info__title"]}>{blog.attributes.titolo.replaceAll("\\n", "\n")}</h3>
										<svg className={style["blog-info__seperator"]}>
											<line x1="0" y1="0" x2="100%" y2="0" />
											<line x1="0" y1="100%" x2="100%" y2="100%" />
										</svg>

										<div className={style["blog-info__source"]}>{(blog.attributes.arte ? blog.attributes.arte + " / " : "") + (blog.attributes.data !== "0000-00-00 00:00:00" ? this.formatDate(new Date(blog.attributes.data)) : "")}</div>
									</div>
								</a>
							</div>
						))}
						<div className={style["blog-arrows"]}>
							<a href="#!" className={style["blog-arrows__image-up"] + " " + ((this.props.blogs != null && this.props.blogs.length > VISIBLE_BLOGS_COUNT && this.state.blogIndex > 0) ? "" : style["blog-arrows__image--unvisible"])} onClick={this.arrowUpClickHandler}><ArrowImage /></a>
							<a href="#!" className={style["blog-arrows__image-down"] + " " + ((this.props.blogs != null && this.props.blogs.length > VISIBLE_BLOGS_COUNT && this.state.blogIndex !== this.props.blogs.length - VISIBLE_BLOGS_COUNT) ? "" : style["blog-arrows__image--unvisible"])} onClick={this.arrowDownClickHandler}><ArrowImage /></a>
						</div>
					</div>
				</div>
			</div>
